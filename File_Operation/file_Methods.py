...
class File_Operation:
        ...
        try:
            ...
            if os.path.isdir(path):  # remove previously existing models for each cluster
                shutil.rmtree(self.model_directory)
                    # Creating directorey with same name as our best model name
                os.makedirs(path)
            else:
                os.makedirs(path)  #

            # Stroing our model in perticuler that folder location using pickle with file extantion .sav
    ...
    def find_correct_model_file(self, cluster_number):
        ...
        Description: Select the correct model based on the cluster number
        """
